...
// 以下、メイン処理.リジッドボディと絡めるので、FixedUpdate内で処理を行う.
	void FixedUpdate ()
	{
        ...
        audioSources[1].volume = Mathf.Lerp(0.0f, 1.0f, v);
		...
		if (Input.GetButtonDown("Jump") && controlable) {	// スペースキーを入力したら
    ...
    public void Controlable(bool b) {
        controlable = b;
    }

    public void StageFinish(int i) {
        controlable = false;
        if (i == 0) Invoke("GoalMotion", 1.0f); // Goal
        if (i == 1) Invoke("GameOverMotion", 1.0f);
    }

    void GoalMotion()
    {
        goalCameraScript.GoalCameraMotion(0);
    ...
    void GameOverMotion()
    {
        goalCameraScript.GoalCameraMotion(1);
